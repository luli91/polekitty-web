...
const CalendarioMensual = ({ setToast }: { setToast: (toast: { mensaje: string; tipo?: "exito" | "error" | "info" }) => void }) => {
  const { user } = useAuth();
              ...
              >
                ...
                <div className="grid grid-cols-7 gap-1 sm:gap-2">
                  {dias.map((fecha, index) => {
                    ...
                    const hayCupo = clasesDelDia.some(c => (c.cupoMaximo ?? 0) > (c.anotadas?.length ?? 0));
      ...
      {diaSeleccionado && (
        <ModalReservaClase
          ...
          clases={clases.filter(c => {
            const fechaClase = c.fecha?.toDate?.();
            if (!fechaClase) return false;
            return formatearFecha(fechaClase) === diaSeleccionado &&
